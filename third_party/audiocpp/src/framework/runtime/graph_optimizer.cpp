...
GraphOptimizationReport optimize_graph(
    ...
    // Compatibility with ckadirt/ggml 70081fdf, before graph_set_n_nodes was
    // added: retain the compacted nodes through the public clear/add API.
    std::vector<ggml_tensor *> retained(nodes, nodes + write_index);
    ggml_graph_clear(&graph);
    for (ggml_tensor * node : retained) {
