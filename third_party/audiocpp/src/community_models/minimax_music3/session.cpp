...
std::shared_ptr<const MiniMaxMusic3Assets> select_component_assets(
    ...
    if (const auto value = runtime::find_option(options, {"minimax_music3.condition_encoder_gguf"})) {
        selected->condition_encoder_weights = assets::open_tensor_source(resolve_component_gguf_path(
            *base,
            "minimax_music3.condition_encoder_gguf",
            *value));
        changed = true;
    }
    ...
    if (const auto value = runtime::find_option(options, {"minimax_music3.vocoder_gguf"})) {
        selected->vocoder_weights = assets::open_tensor_source(resolve_component_gguf_path(
            *base,
            "minimax_music3.vocoder_gguf",
            *value));
        changed = true;
    }
    if (!changed) {
...
runtime::ModelCliInterface minimax_music3_cli_interface() {
    ...
    out.session_options = {
        ...
        {"minimax_music3.language_model_gguf", "string", "Language model component GGUF file relative to the model root.", false, "lm-Q4_K_M.gguf"},
        {"minimax_music3.rvq_depth_decoder_gguf", "string", "RVQ depth decoder component GGUF file relative to the model root.", false, "rvq-Q4_K_M.gguf"},
        {"minimax_music3.condition_encoder_gguf", "string", "Condition component file relative to the model root.", false, "condition-F32.gguf"},
        {"minimax_music3.flow_transformer_gguf", "string", "Flow transformer component GGUF file relative to the model root.", false, "dit-Q4_K_M.gguf"},
        {"minimax_music3.vocoder_gguf", "string", "Vocoder component file relative to the model root.", false, "vae-F16.gguf"},
